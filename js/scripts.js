  ...
  function AddListItem(artist, song) {
    ...
    fetch(ApiUrl)
      ...
      .then(function (json) {
        ...
        artistSong.textContent =
          `${typedArtist.join("").toUpperCase()}` +
          " : " +
          `${typedSong.join("").toUpperCase()}`;
        ...
        btn.addEventListener(
          ...
          function (e) {
            let string =
              ...
              "<br>" +
              `<span class="highlight">${artistSong.textContent}</span>`;
            document.getElementsByClassName(
              "modal-title"
            )[0].innerHTML = string;
            document.getElementsByClassName("delete")[0].addEventListener(
              "click",
              function () {
...
document
  .getElementById("searchforartist")
  .addEventListener(
    "input",
    (event) => songRepository.searchForSongArtist(event),
    false
  );
document
  .getElementById("searchforsong")
  .addEventListener(
    "input",
    (event) => songRepository.searchForSong(event),
    false
  );
document
  .getElementById("search-icon")
  .addEventListener("click", songRepository.validateInfo, false);
